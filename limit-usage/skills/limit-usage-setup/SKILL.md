...
Pick `install` or `uninstall` from the argument. Run guard.sh with both the session id and the data dir (the data dir is where the wrapper copy lives):
`CLAUDE_PLUGIN_DATA=${CLAUDE_PLUGIN_DATA} CLAUDE_SESSION_ID=${CLAUDE_SESSION_ID} CLAUDE_PLUGIN_ROOT=${CLAUDE_PLUGIN_ROOT} ${CLAUDE_PLUGIN_ROOT}/bin/guard.sh <install|uninstall>`
...
6. Guard activates after the next response (first statusLine refresh writes the snapshot). Set thresholds with `/limit-usage set`.
