...
**既定 (grok-4.3 / `--max-turns 5`) で 1 クエリ $0.5-0.8**。公開価格表から計算すると
$0.12 程度だが、実請求はその 8 倍近くになる (差分は x_search のツール課金)。
実額は出力 JSON の `usage.cost_in_usd_ticks / 1e9` で毎回確認する。

コストは `--max-turns` (エージェントループの上限) にほぼ比例する。適切な値は
話題のポスト量次第で事前には決まらないため、既定の 5 で投げて、足りなければ
上げて投げ直す運用にしている (`--max-turns 2` で $0.4 前後、`10-12` で $0.8-1.0)。
