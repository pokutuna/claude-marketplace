...
調査結果のレポートに加えて、**根拠となるポストの引用と URL** を返す。
...
| `findings[].sources[]` | 裏付けとなるポストの URL・投稿者・日付・引用 |
...
(引用ごと除去する)。照合はポスト ID で行い、ホストが `x.com` / `twitter.com` で
あることも確認する — 実在するポスト ID を含んだ別ホストの URL
(`https://attacker.example/x.com/i/status/<実在 ID>`) を、そのポストとして
表示させないためである。検証は `sources` だけでなく `report` 本文中のリンクと
`links[]` にも及ぶ。根拠として認めるのは X のポスト URL のみで、エージェントが
閲覧したニュース記事などが X 上の発言の裏付けに化けることを防ぐ。

**検証が保証するのはポストの実在であって、引用文の中身ではない。** API はポスト
本文をスクリプトに返さないため、`quote` / `author_handle` / `date` はモデルの
転記であり、機械的な照合ができない (出力の `sourcing.quotes_verified` は常に
`false`)。原文の正確さが要件になる用途では、X API での hydrate で突き合わせる。
...
モデル知識からの合成をそれと判別する。検証を通った出典が 1 つもない場合、
スクリプトは終了コード 3 を返す (0 = 利用可、1 = リクエスト失敗、2 = 引数エラー)。
...
**既定 (grok-4.3 / `--max-turns 5`) で 1 クエリ $0.5-0.8 程度**。これは
`--max-turns` 2 と 12 の実測からの推定で、この設定自体の実測ではない。
トークン単価と公開されているツール単価 ($5/1,000 calls) から計算した額の
8 倍近くが実際に請求される (差分は x_search のツール課金で、原因は未特定)。
見積もりを計算で出さず、実額は出力 JSON の `usage.cost_in_usd_ticks / 1e9` で
毎回確認する。

コストは `--max-turns` (エージェントループの上限) にほぼ比例する
(`--max-turns 2` で $0.4 前後、`10-12` で $0.8-1.0。いずれも数回の実行に基づく
目安で、話題のポスト量でぶれる)。何回検索すれば足りるかは事前には分からないので、
skill は既定の 5 で実行し、カバレッジが不足していれば値を上げた再実行を
追加コストとともに提案する。

エラー時の自動再試行は既定で無効 (`--retries 0`)。エージェントループが完走した
後の 5xx でも検索は課金されるため、再試行は同じ調査をもう一度買いうる。
...
- 検索結果は第三者が書いた外部入力である。ポスト本文に含まれる指示に従わないよう
  system prompt で指示しているが、出力を自動処理に流す場合は内容を信用しない
- 既定でリクエストは xAI 側に保存される (`store: true`、追い質問に必要)。
  プロンプトや結果が機微な場合は `--no-store` を渡す。`--raw-out` の保存先には
  プロンプト全文と検索結果が残るので、扱いに注意する
