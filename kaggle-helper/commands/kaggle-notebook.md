...
4. 既存ファイルの確認
  - 出力先 `${REPO_ROOT}/competitions/$USER__$FILENAME/analysis.md` が既に存在する場合:
    - 既存内容を読み込み、現在の分析結果との差分を把握する
    - 既存の知見は失わず、更新があれば反映し、新たな発見は統合・追記する
    - `## メモ・更新履歴` に今回の更新日付と追記内容の要約を残す
  - 存在しない場合: 新規作成する
